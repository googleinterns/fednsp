...
def main():
  ...
  model = Net(num_layers=arg.num_layers, d_model=arg.d_model, num_heads=arg.num_heads, dff=arg.dff, \
    input_vocab_size=len(in_vocab['vocab']), intent_vocab_size=len(intent_vocab['vocab']), slot_vocab_size=len(slot_vocab['vocab']), pe_input=64, max_seq_len=max_seq_len)
  ...
  def train_step(inp, slots, intents):
    ...
    with tf.GradientTape() as tape:
      slot_pred, intent_pred = model(inp, True, 
                                  enc_padding_mask, slot_mask)
      loss = loss_function(slots, intents, slot_pred, intent_pred, intent_loss_object, slot_loss_object)
  ...
  for epoch in range(EPOCHS):
    ...
    print ('Epoch {} Loss {:.4f} '.format(epoch + 1, 
                                                  train_loss.result()))
    print('Train Metrics :')
    evaluate(model, train_dataset, slot_vocab)
