...
from tensorflow_federated.python.learning import federated_averaging_with_personalization as federated_averaging
...
class FederatedAveragingClientWithModelTest(test.TestCase,
  ...
  def test_client_tf(self):
    ...
    dataset['train'] = self.create_dataset()
    dataset['test'] = self.create_dataset()
...
class FederatedAveragingModelTffTest(test.TestCase, parameterized.TestCase):
  ...
  def test_fails_stateful_broadcast_and_process(self):
    ...
    with self.assertRaises(optimizer_utils.DisjointArgumentError):
      federated_averaging.build_federated_averaging_personalization_process(
  ...
  def test_basic_orchestration_execute(self):
    iterative_process = federated_averaging.build_federated_averaging_personalization_process(
        model_fn=model_examples.LinearRegression,
        client_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=0.1))

    ds = collections.OrderedDict()
    ds['train'] = tf.data.Dataset.from_tensor_slices(
        collections.OrderedDict(
            x=[[1.0, 2.0], [3.0, 4.0]],
            y=[[5.0], [6.0]],
        )).batch(2)
    ds['test'] = tf.data.Dataset.from_tensor_slices(
        collections.OrderedDict(
            x=[[1.0, 2.0], [3.0, 4.0]],
            y=[[5.0], [6.0]],
        )).batch(2)        
  ...
  def test_orchestration_execute_from_keras(self, build_keras_model_fn):
    ...
    def model_fn():
      ...
      return keras_utils.from_keras_model(
          ...
          input_spec=ds['train'].element_spec,
