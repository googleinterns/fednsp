...
def train_model(model, arg, train_dataset, valid_dataset, slot_vocab):
    ...
    checkpoint_manager = manage_checkpoints(model,
    ...
    def train_step(inputs, slots, intents):
        enc_padding_mask, intent_mask = create_masks(inputs)

        with tf.GradientTape() as tape:
            slot_pred, intent_pred = model(inputs, True, enc_padding_mask,
                                           intent_mask)
            loss = loss_function(slots, intents, slot_pred, intent_pred,
                                 intent_loss_objective, slot_loss_objective)
    ...
    for epoch in range(arg.max_epochs):
        ...
        for inputs, slots, intents in train_dataset:
            train_step(inputs, slots, intents)
