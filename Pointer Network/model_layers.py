...
class BiliearAttention(tf.keras.layers.Layer):
    ...
    def call(self, key, query, mask):
        ...
        attention_score = tf.keras.layers.Dot(axes=[2, 2])([query, weighted_keys]) #(bs, output_seq_len, input_seq_len)
...
class OutputHead(tf.keras.layers.Layer):
    ...
    def call(self, encoder_outputs, decoder_outputs, pointer_mask):
        ...
        intent_slot_outputs = self.intent_slot_layer(decoder_outputs) # (batch_size, output_seq_len, intent_slot_vocab_size)
        pointer_outputs = self.pointer_layer(encoder_outputs, decoder_outputs, pointer_mask) # (batch_size, output_seq_len, input_seq_len)
