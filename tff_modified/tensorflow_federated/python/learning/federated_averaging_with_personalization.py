...
from tensorflow_federated.python.learning.framework import optimizer_utils_p13n as optimizer_utils
...
class ClientFedAvg(optimizer_utils.ClientDeltaFn):
  ...
  def __call__(self, dataset, initial_weights):
    ...
    @tf.function
    def reduce_fn_test(dummy_input, batch):
      """Train `tff.learning.Model` on local client batch."""
      output = model.forward_pass(batch, training=False)
      return dummy_input

    num_examples_sum = dataset['train'].reduce(
        initial_state=tf.constant(0), reduce_func=reduce_fn_train)
    ...
    dataset['test'].reduce(
        initial_state=tf.constant(0), reduce_func=reduce_fn_test)   
    aggregated_test_outputs = model.report_local_outputs()
    # for key, value in aggregated_test_outputs.items():
    ...
    if has_non_finite_delta > 0:
      ...
      weights_delta_weight = self._client_weight_fn(aggregated_test_outputs)

    return optimizer_utils.ClientOutput(
        weights_delta, weights_delta_weight, aggregated_test_outputs,
...
def build_federated_averaging_personalization_process(
