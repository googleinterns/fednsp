...
def generate_iid_splits(in_data, slot_data, intent_data, num_clients=10):
    ...
    fed_data = tff.simulation.FromTensorSlicesClientData(fed_data)
...
def generate_splits_type3(in_data, slot_data, intent_data, instance_types_per_client=3, clients_per_instance_type=3):
    """Creates non-IID splits of the dataset. Each client is given only a fixed number
    ...
    client_intent_map = collections.defaultdict()
    num_clients = int(np.ceil(len(unique_intents)/float(instance_types_per_client)))*clients_per_instance_type
    
    client_list = []
    for _ in range(int(np.ceil(clients_per_instance_type*len(unique_intents)/num_clients))):
    ...
    for idx, intent_id in enumerate(unique_intents):
        ...
        client_ids = client_list[idx*clients_per_instance_type:(idx+1)*clients_per_instance_type] 
        
        intent_client_distribution = np.random.randint(low=0, high=1000, size=clients_per_instance_type).astype(np.float)
        intent_client_distribution /= np.sum(intent_client_distribution)
        ...
        for i, client_id in enumerate(client_ids):
            client_idxs[client_id] += intent_idxs[(client_idx_distribution == i)].tolist()
