...
class Net(tf.keras.Model):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, 
               slot_vocab_size, intent_vocab_size, pe_input, max_seq_len=46, rate=0.1):
  ...
  def call(self, inp, training, enc_padding_mask, slot_mask):

    enc_output = self.encoder(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
    
    slot_output = self.slot_head(enc_output, slot_mask)  # (batch_size, tar_seq_len, slot_vocab_size)
    intent_output = self.intent_head(enc_output)  # (batch_size, tar_seq_len, intent_vocab_size)
